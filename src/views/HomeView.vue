	...
	<div class="w-screen h-screen bg bg-[#F4F4F4] px-[35px] py-[25px] font-nonto">
		<div class="flex flex-row justify-between items-center w-[100%] h-[75px] mb-[15px]">
			...
			<div class="flex flex-row gap-[15px] h-[75%]">
				<div class="itbkk-manage-status itbkk-button-home cursor-pointer bg bg-[#F9F9F9] border border-[#BDBDBD] rounded-[4px] w-[200px] h-[40px] m-[7px] py-[7px] px-[5px] text-center"
					@click="toggleMode">
					<h1>{{ toggleManage }}</h1>
				</div>
				<div
					...
					<div class="dropdown dropdown-bottom">
						<div tabindex="0" role="" class="cursor-pointer w-full h-full flex items-center">
							<div v-if="statusFilter.length === 0">Filter
								by
								status(es)
							</div>
							<div v-else class="flex gap-[10px] border border-blue-500 w-[150px] overflow-auto"
								tabindex="0" role="">
								<div v-for="status in statusFilter"
									class="flex gap-[10px] border border-red-500 min-w-[50px]">
									<div>
										{{ status }}
									</div>
									<div @click="filterDeleteSelection(status)" class="z-[10]">X</div>
								</div>
							</div>
						</div>
						<ul tabindex="0"
							class="dropdown-content z-[1] menu shadow bg-base-100 rounded-[10px] w-[150px]">
							<li v-for="status in statusManagement.getAllStatus()" class="break-all"
								@click="filterSelection(status.name)"><a>{{
		status.name
	}}</a></li>
